...
- **高级模式**：结构化理解——大类/细类、品牌、型号、颜色、可见文字、外观特征、成色。每个字段都标注"确认/疑似/无法确定"三种状态之一；不确定的地方绝不编造。外观描述只写"看到了什么"（颜色、形状、位置），不对痕迹/污渍的成因下诊断性结论（例如不会写"血迹""烧灼痕迹"，那是专业鉴定人员的工作）。可见文字由 Qwen3-VL 初判后再经本地 PaddleOCR 交叉校验（"VLM 负责看懂，OCR 负责看清字"）。同样支持上传同一物品的多张角度照片自动合并。
...
这个文件夹不是本项目代码创建的，也不清楚具体用途——里面是约 75 张 PNG 图片，时间跨度从 2024 年到 2026 年初不等。清理垃圾文件时特意没有动它，如果确认不需要了，麻烦告诉我一声我再处理。
